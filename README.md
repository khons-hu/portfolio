...
The separate local Python tool refreshes a private rotation daily at 18:00 when the Mac is available. One weekly Jev request uses approved track and artist names to choose a musical direction and a small discovery share. Daily refreshes reuse the cached choice and verify the saved playlist. No scheduled Codex or Claude sessions are needed. Mood matching is approximate, not audio analysis. The public night playlist linked on this site is separate from the private scheduled rotation.

## Listening now (Spotify)

`api/now-playing.js` is a Vercel Node function that asks Spotify what is playing and returns only the track or podcast episode title, artists or show name, its `open.spotify.com` link, and the track's duration and playback position. It never returns history, artwork, device, playlist or context, and it refuses tokens with anything beyond read-only playback scopes. A null, missing, negative, string or non-finite position is sent as `null` (never 0) and a position past the end is clamped. Playing and idle answers are cached on the CDN for 5 seconds (up to 5 minutes when rate-limited or unconfigured), so visitors share one Spotify request per interval. `now-playing.js` keeps a quiet Spotify summary in On my desk. It says "Listening now" with the track only while a track is confirmed playing and fresh, otherwise "Offline", "Updating…" (between songs) or "Currently unavailable"; paused, private session, ads, local files, errors and anything older than the track's end or 150 seconds never show as playing. Before the first answer the status stays blank rather than guessing. The summary keeps the same two rows in every state (the status takes the place of the bar). Between songs ("Updating…") the first row also keeps the height it had while playing, so a title that wraps on a phone does not collapse and grow back. The page below moves only when the next title needs a different number of lines, or on a phone when a two-line title gives way to "Offline". The status is not a live region, so ambient updates never interrupt a screen reader. The page polls every 10 seconds while playing and every 15 seconds otherwise, only while the tab is visible. A request that has not settled after 8 seconds is abandoned; a failed check keeps a still-fresh track and retries at the idle pace.

Below the line, a thin read-only bar shows elapsed and total time, like a phone's mini player. It is estimated on the page: the answer's position plus the CDN `Age` header plus half the request's round trip, counted on the monotonic clock (`performance.now`), never the visitor's wall clock. It steps once a second without transition and only while the tab is visible, clamps at the total, and disappears when the last answer is more than 90 seconds old, so it never runs on forever. A pause or seek between checks is only seen at the next check (every 10 seconds while playing), so the position is approximate; the bar's tooltip and accessible value say so. At the track's expected end the page checks once for the next track; a stale CDN copy of the same track cannot trigger more checks. Hidden tabs keep no timers and make no requests.

"Listen here" loads Spotify's official track embed (`https://open.spotify.com/embed/track/<id>`, or the corresponding episode URL, built only from a validated ID) after a click. Nothing is requested from Spotify before that click, and the frame has no autoplay permission, so nothing plays until the visitor presses play inside it. Spotify decides per visitor whether that is a preview or the full track. The player is separate from Patrick's live line: polls never reload or switch it, and it stays open if Patrick stops. The button is a disclosure toggle ("Listen here" / "Close player", with a stop square while open); reopening loads the track that is playing then, or the last one the visitor opened. "Close player" removes the frame, which stops playback, and returns focus. Below 360px the toggle is a round icon button so the bar keeps its row; its label stays available to assistive technology. "Open in Spotify" stays available. The Content Security Policy allows frames only from `https://open.spotify.com`.
