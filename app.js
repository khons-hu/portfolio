...
const projects = {
  ...
  portfolio: { title: 'This little corner of the web', category: 'CURRENT PROJECT · HTML / CSS / JAVASCRIPT', description: 'My personal portfolio, built with coding-agent assistance and plain browser tools.', notes: ['Searchable projects, shareable project notes, terminal navigation and a small guide in multiple languages. It answers with an AI model through Groq when available and falls back to prepared answers.', 'Light and dark themes, reduced-motion support, and animations that pause when the tab is hidden. No frontend framework or model download.', 'Deployed on Vercel, with updates published from GitHub. The source is public on GitHub.'], url: 'https://github.com/khons-hu/portfolio' },
  rotation: {"title": "Spotify rotation", "category": "PERSONAL EXPERIMENT · PYTHON / SPOTIFY API", "description": "A personal rotation and mood playlists on request. Built locally with Python.", "playlist": "https://open.spotify.com/playlist/3TT6VlZMa8kB3aGCsIulV8?si=c600bcff64164b14", "linkLabel": "Open playlist ↗", "linkKind": "external", "notes": ["A private Python rotation refreshes daily at 18:00 when my Mac is available. Once a week, Jev uses track and artist names to choose a direction and a few discoveries. Daily updates reuse that choice.", "On request, it also searches by mood, activity or artist and favours familiar artists in the results. Mood matching is approximate, not audio analysis.", "The playlist is saved privately and checked after writing. This is a personal tool running on my Mac. The source is not published."]},
...
function showProject(id) {
  ...
  const primaryUrl = liveUrl || project.playlist || null;
  link.hidden = !primaryUrl;
  if (primaryUrl) link.href = primaryUrl; else link.removeAttribute('href');
  link.textContent = t(project.linkLabel || 'Open app ↗');
  link.setAttribute('data-kind', project.linkKind || 'live');
